...
@app.route('/thread')
def render_thread():
    return render_template('thread.html',threadName = request.args('threadName'), posts= "hello") #format_docs(request.args('threadName'))
...
def get_threads():
    ...
    for collection in myList:
        toReturn += Markup("<input type='radio' onselect='myFunction()' name = 'threadName' value='" + collection + "'>" + collection + "<br>")
    return toReturn
